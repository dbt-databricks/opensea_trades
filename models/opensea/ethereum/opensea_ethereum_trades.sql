...
select
    ...
    case
        when erc1155_tokens_in_tx.num_of_items >=1
            then 'erc1155'
        when erc721_tokens_in_tx.num_of_items >=1
            then 'erc721'
        else w.erc_standard
    end as erc_standard,
    ...
    case
        when agg.name is null and erc721_tokens_in_tx.num_of_items > 1
            then erc721_tokens_in_tx.num_of_items
        when agg.name is null and erc1155_tokens_in_tx.num_of_items > 1
            then erc1155_tokens_in_tx.num_of_items
        when w.trade_type = 'Single Item Trade'
            then 1
        when w.erc_standard = 'erc1155'
            then erc721_tokens_in_tx.num_of_items
        when w.erc_standard = 'erc721'
            then erc1155_tokens_in_tx.num_of_items
        else
            (select count(1)
            from erc721_token_transfers
    ...
    case
        when agg.name is not null
            then 'Aggregator Trade'
        when erc721_tokens_in_tx.num_of_items = 1 or erc1155_tokens_in_tx.num_of_items = 1
            then 'Single Item Trade'
        when erc721_tokens_in_tx.num_of_items > 1 or erc1155_tokens_in_tx.num_of_items > 1
            then 'Bundle Trade'
        else w.trade_type
    end as trade_type,
    ...
    case
        when agg.name is not null
            then agg_tokens.symbol
        else nft_tokens.name
    end as nft_project_name,
    -- Adjust the currency amount/symbol with erc20 tokens
    {{ utils.displayed_amount('w.currency_amount', 'erc20.decimals') }} as currency_amount,
    {{ utils.displayed_amount('w.currency_amount', 'erc20.decimals') }} * p.price as usd_amount,
    {{ utils.displayed_amount('w.currency_amount', 'erc20.decimals') }} * p.price / pe.price as eth_amount,
